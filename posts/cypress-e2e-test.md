...
프로필 등록 플로우 전체 실제 사용자 시나리오(로그인 → 프로필 등록 입력 → 저장)를 그대로 재현함으로써 필드가 늘어나도 자동화하며 수동 QA 의존도를 낮추는 것이 1차 목표였습니다.
...
- QA 자동화로 필드 추가 시 검증 시간 감소
- Custom Commands 적용으로 테스트 유지보수성 향상
- 기능 · 디자인 변경 시 발생하는 UI 오류 조기 발견

## 러닝 포인트

처음엔 설정에 시간을 들이느니 그냥 직접 눌러보는 게 더 빠른 거 아닐까 싶었습니다. 그런데 한 번 틀을 만들어 놓고 나니까, 필드가 하나 늘어날 때마다 반복해서 확인하던 과정이 거의 손이 안 가게 되어 생각보다 효과가 컸습니다. 테스트를 돌리다가 새로 추가한 UI가 깨진 것도 바로 확인할 수 있었고, 기능 자체가 정상인지 보는 것만으로는 놓칠 부분도 잡을 수 있었습니다.

다만 모든 흐름을 E2E로 덮는 건 비효율적이라고 느꼈습니다. 비용이 큰 만큼, 실제 유저가 거치는 핵심 동선 위주로 선택해서 쓰는 게 더 현실적입니다. 전체를 자동화한다기보다, 중요한 부분을 지켜주는 안전망을 하나 만들어두는 느낌에 가깝습니다.
